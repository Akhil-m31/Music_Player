...
function renderSongList(filteredSongs = queue, reset = true) {
    ...
    const toRender = filteredSongs.slice(renderedCount, renderedCount + CHUNK)
    toRender.forEach((s, i) => {
        const li = document.createElement("li")
        li.draggable = true
        li.innerHTML = `${s.title} - ${s.artist} <button data-id="${renderedCount + i}">＋</button>`
        ...
        li.addEventListener("dragover", e => {
            ...
            if (e.clientY < offset) parent.insertBefore(dragging, li)
            else parent.insertBefore(dragging, li.nextSibling)
        })
...
songList.addEventListener("scroll", () => {
    if (songList.scrollTop + songList.clientHeight >= songList.scrollHeight - 20) {
        renderSongList(queue, false)
    }
})
...
// ------------------- NEXT SONG LOGIC -------------------
function nextSong() {
    if (isShuffle) index = Math.floor(Math.random() * queue.length)
    else {
        index++
